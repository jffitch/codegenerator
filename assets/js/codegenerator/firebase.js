...
function firebaseCode() {
    let locations = getRows("locations").split("|").filter(v => v.length == 4).map(v => {
            return {database:v[0].replace(/(^[^\/\*]*)\*?$/g, (_, a) => _.includes("*") ? `Constants.${a.toUnderscore().toUpperCase()}` : `"${a}"`).split("/").map(v => `.child(${v})`).join(""),
            object:v[1].toCamel(),
            keyword:v[2].toCamel(),
...
`;
    for (let i of locations) {
        code += `
fun create${i.keyword.capitalize()}(${i.variablesInline}) {
...
fun update${i.keyword.capitalize()}(itemKey: String, ${i.variablesInline}) {
...
`;
        for (let j of i.variablesList) {
            code += `
fun update${i.keyword.capitalize()}${j.variable.capitalize()}(itemKey: String, ${j.variable}: ${j.dataType}) {
    val itemReference = database${i.database}.child(itemKey)
    itemReference.child("${j.variable}").setValue(${j.variable})
}
`;
        ...
        code += `
fun delete${i.keyword.capitalize()}(itemKey: String) {
    val itemReference = database${i.database}.child(itemKey)
